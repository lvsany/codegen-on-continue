...
def parse_pytest_summary(output: str):
    ...
    return {"error": error, "passed": passed, "failed": failed, "skipped": skipped}
...
class Test:
    ...
    def check_tests(self, path, language, test_mode):
        ...
        return check_output, error, passed, total

    def test(self, path, language):
        self.setup_implementation()
        check_test_output, error, passed, total = self.check_tests(path, language, "check_tests")
        if error:
